...
from .base_sam_mask_head import BaseSamMaskHead
from .fcos_sam_head import FCOSSamHead

INF = 1e8


@MODELS.register_module()
class CondInstSamBboxHead(FCOSSamHead):
    ...
    def forward_single(self, x: Tensor, scale: Scale,
        ...
        cls_score, bbox_pred, cls_feat, reg_feat = \
            super(FCOSSamHead, self).forward_single(x)
    ...
    def loss_by_feat(
        ...
        if len(pos_inds) > 0:
            ...
            # print(pos_decoded_target_preds)
        ...
        self._raw_positive_infos.update(bbox_preds=bbox_preds)
    ...
    def get_positive_infos(self) -> InstanceList:
        ...
        point_list = []
        stride_list = []
        for cls_score_per_lvl, bbox_pred_per_lvl, centerness_per_lvl, param_pred_per_lvl,\
            point_per_lvl, stride_per_lvl in \
            zip(self._raw_positive_infos['cls_scores'],
                self._raw_positive_infos['bbox_preds'],
            ...
            bbox_pred_per_lvl = \
                bbox_pred_per_lvl.permute(
                    0, 2, 3, 1).reshape(num_imgs, -1, 4)
            centerness_per_lvl = \
            ...
            bbox_pred_list.append(bbox_pred_per_lvl)
        ...
        bbox_preds = torch.cat(bbox_pred_list, dim=1)
        ...
        bbox_proposals = self.bbox_coder.decode(
            all_points, bbox_preds * all_strides.unsqueeze(2).repeat(1, 1, 4))
        max_num = 180
        positive_infos = []
        for i, (pos_gt_inds,
                pos_inds) in enumerate(zip(pos_gt_inds_list, pos_inds_list)):
            pos_inds = pos_inds[:max_num]
            pos_gt_inds = pos_gt_inds[:max_num]
            ...
            # pos_info.bbox_preds = bbox_preds[i][pos_inds]
            pos_info.bboxes = bbox_proposals[i][pos_inds]
...
@MODELS.register_module()
class CondInstSamMaskHead(BaseSamMaskHead):
    ...
    def _init_layers(self) -> None:
        ...
        for i in range(self.num_layers):
            if i == 0:
                weight_nums.append((self.in_channels + 6) * self.feat_channels)
    ...
    def gen_mask(self, rois, image_embeddings, multimask_output=False):
        ...
        mask_features, _ = self.mask_decoder(
        ...
        # mask_features = mask_features > 0.0
        N, C, H, W = mask_features.size()
        mask_features = mask_features.reshape(N, C, H // 2, 2, W // 2, 2)
        mask_features = mask_features.permute(0, 3, 5, 1, 2, 4)
        mask_features = mask_features.reshape(N, C * 4, H // 2, W // 2)
        return mask_features

    def forward(self, x: tuple, positive_infos: InstanceList, image_embeddings: Tensor) -> tuple:
        ...
        return multi_apply(self.forward_single, mask_feats, positive_infos, image_embeddings)

    def forward_single(self, mask_feat: Tensor,
                       positive_info: InstanceData,
                       image_embeddings: Tensor) -> Tensor:
        ...
        pos_proposals = positive_info.get('bboxes')

        num_inst = pos_param_preds.shape[0]
        # print(pos_param_preds.shape)
        ...
        mask_features = self.gen_mask(pos_proposals, image_embeddings, multimask_output=False)
        mask_head_inputs = torch.cat([rel_coords, mask_feat, mask_features], dim=1)
