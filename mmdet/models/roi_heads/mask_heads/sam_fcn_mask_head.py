...
class SAMFCNMaskHead(BaseModule):
    ...
    def forward(self, x: Tensor, mask_feats: Tensor = None) -> Tensor:
        ...
        """
        # mask_feats = self.conv_res(mask_feats)
        # x = x + mask_feats
        x = torch.cat([x, mask_feats], dim=1)
