    ...
    def prepare_targets_cr(self, proposals, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            new_matched_targets = matched_targets.copy_with_fields(
                ["matched_idxs", "labels"])
            # print(pos_inds)
            # print(new_matched_targets.bbox)
...
def make_roi_mask_loss_evaluator(pos_iou_thr=0.5, neg_iou_thr=0.5, mask_size=28):
    matcher = Matcher(pos_iou_thr, neg_iou_thr, allow_low_quality_matches=False)
